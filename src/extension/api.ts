...
import { IpcServer } from "@kit-pilot/ipc"
...
export class API extends EventEmitter<KitPilotEvents> implements KitPilotAPI {
	...
	constructor(
		...
		if (enableLogging) {
			...
			this.logfile = path.join(os.tmpdir(), "kit-pilot-messages.log")
		} else {
			this.log = () => {}
		}
		...
		if (socketPath) {
			...
			ipc.on(IpcMessageType.TaskCommand, async (clientId, command) => {
				const sendResponse = (eventName: KitPilotEventName, payload: unknown[]) => {
					ipc.send(clientId, {
				...
				switch (command.commandName) {
					...
					case TaskCommandName.GetCommands:
						try {
							...
							sendResponse(KitPilotEventName.CommandsResponse, [
								commands.map((cmd) => ({
									name: cmd.name,
									source: cmd.source,
					...
					case TaskCommandName.GetModes:
						try {
							...
							sendResponse(KitPilotEventName.ModesResponse, [modes])
						} catch (error) {
							sendResponse(KitPilotEventName.ModesResponse, [[]])
						}
					...
					case TaskCommandName.GetModels:
						try {
							...
							sendResponse(KitPilotEventName.ModelsResponse, [
	...
	public override emit<K extends keyof KitPilotEvents>(
		eventName: K,
		...args: K extends keyof KitPilotEvents ? KitPilotEvents[K] : never
	) {
		const data = { eventName: eventName as KitPilotEventName, payload: args } as TaskEvent
	...
	private registerListeners(provider: ClineProvider) {
		provider.on(KitPilotEventName.TaskCreated, (task) => {
			// Task Lifecycle

			task.on(KitPilotEventName.TaskStarted, async () => {
				this.emit(KitPilotEventName.TaskStarted, task.taskId)
				await this.fileLog(`[${new Date().toISOString()}] taskStarted -> ${task.taskId}\n`)
			})

			task.on(KitPilotEventName.TaskCompleted, async (_, tokenUsage, toolUsage) => {
				this.emit(KitPilotEventName.TaskCompleted, task.taskId, tokenUsage, toolUsage, {
			...
			task.on(KitPilotEventName.TaskAborted, () => {
				this.emit(KitPilotEventName.TaskAborted, task.taskId)
			})

			task.on(KitPilotEventName.TaskFocused, () => {
				this.emit(KitPilotEventName.TaskFocused, task.taskId)
			})

			task.on(KitPilotEventName.TaskUnfocused, () => {
				this.emit(KitPilotEventName.TaskUnfocused, task.taskId)
			})

			task.on(KitPilotEventName.TaskActive, () => {
				this.emit(KitPilotEventName.TaskActive, task.taskId)
			})

			task.on(KitPilotEventName.TaskInteractive, () => {
				this.emit(KitPilotEventName.TaskInteractive, task.taskId)
			})

			task.on(KitPilotEventName.TaskResumable, () => {
				this.emit(KitPilotEventName.TaskResumable, task.taskId)
			})

			task.on(KitPilotEventName.TaskIdle, () => {
				this.emit(KitPilotEventName.TaskIdle, task.taskId)
			})

			// Subtask Lifecycle

			task.on(KitPilotEventName.TaskPaused, () => {
				this.emit(KitPilotEventName.TaskPaused, task.taskId)
			})

			task.on(KitPilotEventName.TaskUnpaused, () => {
				this.emit(KitPilotEventName.TaskUnpaused, task.taskId)
			})

			task.on(KitPilotEventName.TaskSpawned, (childTaskId) => {
				this.emit(KitPilotEventName.TaskSpawned, task.taskId, childTaskId)
			})

			task.on(KitPilotEventName.TaskDelegated as any, (childTaskId: string) => {
				;(this.emit as any)(KitPilotEventName.TaskDelegated, task.taskId, childTaskId)
			})

			task.on(KitPilotEventName.TaskDelegationCompleted as any, (childTaskId: string, summary: string) => {
				;(this.emit as any)(KitPilotEventName.TaskDelegationCompleted, task.taskId, childTaskId, summary)
			})

			task.on(KitPilotEventName.TaskDelegationResumed as any, (childTaskId: string) => {
				;(this.emit as any)(KitPilotEventName.TaskDelegationResumed, task.taskId, childTaskId)
			})

			// Task Execution

			task.on(KitPilotEventName.Message, async (message) => {
				this.emit(KitPilotEventName.Message, { taskId: task.taskId, ...message })
			...
			task.on(KitPilotEventName.TaskModeSwitched, (taskId, mode) => {
				this.emit(KitPilotEventName.TaskModeSwitched, taskId, mode)
			})

			task.on(KitPilotEventName.TaskAskResponded, () => {
				this.emit(KitPilotEventName.TaskAskResponded, task.taskId)
			})

			task.on(KitPilotEventName.QueuedMessagesUpdated, (taskId, messages) => {
				this.emit(KitPilotEventName.QueuedMessagesUpdated, taskId, messages)
			})

			// Task Analytics

			task.on(KitPilotEventName.TaskToolFailed, (taskId, tool, error) => {
				this.emit(KitPilotEventName.TaskToolFailed, taskId, tool, error)
			})

			task.on(KitPilotEventName.TaskTokenUsageUpdated, (_, tokenUsage, toolUsage) => {
				this.emit(KitPilotEventName.TaskTokenUsageUpdated, task.taskId, tokenUsage, toolUsage)
			})

			// Let's go!

			this.emit(KitPilotEventName.TaskCreated, task.taskId)
		})
	}
	...
	public getConfiguration(): KitPilotSettings {
		return Object.fromEntries(
			Object.entries(this.sidebarProvider.getValues()).filter(([key]) => !isSecretStateKey(key)),
		)
	}

	public async setConfiguration(values: KitPilotSettings) {
