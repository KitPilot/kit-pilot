...
import type {
	KitPilotTerminalProvider,
	KitPilotTerminal,
	KitPilotTerminalCallbacks,
	KitPilotTerminalProcess,
	KitPilotTerminalProcessResultPromise,
	ExitCodeDetails,
} from "./types"

export abstract class BaseTerminal implements KitPilotTerminal {
	public readonly provider: KitPilotTerminalProvider
	...
	public process?: KitPilotTerminalProcess
	public completedProcesses: KitPilotTerminalProcess[] = []

	constructor(provider: KitPilotTerminalProvider, id: number, cwd: string) {
	...
	abstract runCommand(command: string, callbacks: KitPilotTerminalCallbacks): KitPilotTerminalProcessResultPromise
	...
	public setActiveStream(stream: AsyncIterable<string> | undefined, pid?: number): void {
		if (stream) {
			if (!this.process) {
				...
				console.warn(
					`[Terminal ${this.provider}/${this.id}] process is undefined, so cannot set terminal stream (probably user-initiated non-KitPilot command)`,
				)
	 ...
	 */
	public getProcessesWithOutput(): KitPilotTerminalProcess[] {
