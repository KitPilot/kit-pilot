...
import { KitPilotTerminal, KitPilotTerminalProvider } from "./types"
...
export class TerminalRegistry {
	private static terminals: KitPilotTerminal[] = []
		...
		try {
			const startDisposable = vscode.window.onDidStartTerminalShellExecution?.(
				async (e: vscode.TerminalShellExecutionStartEvent) => {
					...
					if (terminal) {
						...
						console.error(
							"[onDidStartTerminalShellExecution] Shell execution started, but not from a KitPilot-registered terminal:",
							e,
						)
					}
				},
			...
			const endDisposable = vscode.window.onDidEndTerminalShellExecution?.(
				async (e: vscode.TerminalShellExecutionEndEvent) => {
					...
					if (!terminal) {
						console.error(
							"[onDidEndTerminalShellExecution] Shell execution ended, but not from a KitPilot-registered terminal:",
							e,
						)
	...
	public static createTerminal(cwd: string, provider: KitPilotTerminalProvider): KitPilotTerminal {
		let newTerminal
	...
	public static async getOrCreateTerminal(
		cwd: string,
		taskId?: string,
		provider: KitPilotTerminalProvider = "vscode",
	): Promise<KitPilotTerminal> {
		const terminals = this.getAllTerminals()
		let terminal: KitPilotTerminal | undefined
	 ...
	 */
	public static getTerminals(busy: boolean, taskId?: string): KitPilotTerminal[] {
	 ...
	 * @returns Array of Terminal objects
	 */
	public static getBackgroundTerminals(busy?: boolean): KitPilotTerminal[] {
		return this.getAllTerminals().filter((t) => {
	...
	private static getAllTerminals(): KitPilotTerminal[] {
		this.terminals = this.terminals.filter((t) => !t.isClosed())
		return this.terminals
	}

	private static getTerminalById(id: number): KitPilotTerminal | undefined {
	 ...
	 */
	private static getTerminalByVSCETerminal(vsceTerminal: vscode.Terminal): KitPilotTerminal | undefined {
