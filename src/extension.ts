...
/**
 * Check if we should auto-open the KitPilot sidebar after switching to a worktree.
...
async function checkWorktreeAutoOpen(
	...
	try {
		...
		if (normalizePath(currentPath) === normalizePath(worktreeAutoOpenPath)) {
			...
			outputChannel.appendLine(`[Worktree] Auto-opening KitPilot sidebar for worktree: ${worktreeAutoOpenPath}`)

			// Open the KitPilot sidebar with a slight delay to ensure UI is ready
			setTimeout(async () => {
				try {
					await vscode.commands.executeCommand("kit-pilot.plusButtonClicked")
				} catch (error) {
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// Allows other extensions to activate once KitPilot is ready.
	vscode.commands.executeCommand(`${Package.name}.activationCompleted`)

	// Implements the `KitPilotAPI` interface.
	const socketPath = process.env.KITPILOT_IPC_SOCKET_PATH
