...
import type { ProviderSettings, ModeConfig, ModelInfo } from "@kit-pilot/types"
import { customToolRegistry, formatNative } from "@kit-pilot/core"
...
export async function buildNativeToolsArrayWithRestrictions(options: BuildToolsOptions): Promise<BuildToolsResult> {
	...
	if (experiments?.customTools) {
		const toolDirs = getKitPilotDirectoriesForCwd(cwd).map((dir) => path.join(dir, "tools"))
