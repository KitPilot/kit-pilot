...
import type { ModeConfig } from "@kit-pilot/types"
...
describe("CustomModesManager", () => {
	...
	const mockRoomodes = path.join(mockWorkspacePath, ".kitpilotmodes")
	...
	describe("getCustomModes", () => {
		it("should handle valid YAML in .kitpilotmodes file and JSON for global customModes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]

			const kitpilotmodesModes = [{ slug: "mode2", name: "Mode 2", roleDefinition: "Role 2", groups: ["read"] }]
			...
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				...
				if (path === mockRoomodes) {
					return yaml.stringify({ customModes: kitpilotmodesModes })
				}
				throw new Error("File not found")
			})
		...
		it("should merge modes with .kitpilotmodes taking precedence", async () => {
			...
			// Should contain 3 modes (mode1 from settings, mode2 and mode3 from kitpilotmodes)
			expect(modes).toHaveLength(3)
			expect(modes.map((m) => m.slug)).toEqual(["mode2", "mode3", "mode1"])

			// mode2 should come from .kitpilotmodes since it takes precedence
		...
		it("should handle missing .kitpilotmodes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
		...
		it("should handle invalid YAML in .kitpilotmodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .kitpilotmodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .kitpilotmodes precedence", async () => {
			...
			let kitpilotmodesContent = { customModes: kitpilotmodesModes }

			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockRoomodes) {
					return yaml.stringify(kitpilotmodesContent)
				}
				if (path === mockSettingsPath) {
			...
			;(fs.writeFile as Mock).mockImplementation(async (path: string, content: string, _encoding?: string) => {
				...
				if (path === mockRoomodes) {
					kitpilotmodesContent = yaml.parse(content)
				}
			...
			// Should update global state with merged modes where .kitpilotmodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Roomodes Mode 1", // .kitpilotmodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .kitpilotmodes file when adding project-specific mode", async () => {
			...
			// Mock .kitpilotmodes to not exist initially
			let kitpilotmodesContent: any = null
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
			...
			// Verify .kitpilotmodes was created with the project mode
			...
			// Verify the content written to .kitpilotmodes
			expect(kitpilotmodesContent).toEqual({
				customModes: [
					expect.objectContaining({
						slug: "project-mode",
						name: "Project Mode",
		...
		describe("importModeWithRules", () => {
			...
			it("should successfully import mode without rules files", async () => {
				...
				;(fs.readFile as Mock).mockImplementation(async (path: string) => {
					...
					if (path === mockRoomodes && kitpilotmodesContent) {
						return yaml.stringify(kitpilotmodesContent)
					}
					throw new Error("File not found")
				})
				...
				expect(fs.writeFile).toHaveBeenCalledWith(
					expect.stringContaining(".kitpilotmodes"),
			...
			it("should import multiple modes at once", async () => {
				...
				expect(result.success).toBe(true)
				expect(kitpilotmodesContent.customModes).toHaveLength(2)
				expect(kitpilotmodesContent.customModes[0].slug).toBe("mode1")
				expect(kitpilotmodesContent.customModes[1].slug).toBe("mode2")
			})
			...
			it("should handle import errors gracefully", async () => {
				...
				// Mock fs.writeFile to work normally for .kitpilotmodes but we won't get there
				;(fs.writeFile as Mock).mockResolvedValue(undefined)
			...
			it("should prevent path traversal attacks in import", async () => {
				...
				// Verify that no files were written outside the .kitpilot directory
				const mockWorkspacePath = path.resolve("/mock/workspace")
				const writtenRuleFiles = writtenFiles.filter((p) => !p.includes(".kitpilotmodes"))
				writtenRuleFiles.forEach((filePath) => {
					const normalizedPath = path.normalize(filePath)
					const expectedBasePath = path.normalize(path.join(mockWorkspacePath, ".kitpilot"))
	...
	customModes:
			...
			it("should remove existing rules folder when importing mode without rules", async () => {
				...
				expect(fs.rm).toHaveBeenCalledWith(expect.stringContaining(path.join(".kitpilot", "rules-test-mode")), {
					recursive: true,
					force: true,
				})
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should return false when mode is not in .kitpilotmodes file", async () => {
			const kitpilotmodesContent = { customModes: [{ slug: "other-mode", name: "Other Mode" }] }
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
		...
		it("should return false when .kitpilotmodes doesn't exist and mode is not a custom mode", async () => {
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
		...
		it("should return false when rules directory doesn't exist", async () => {
			const kitpilotmodesContent = { customModes: [{ slug: "test-mode", name: "Test Mode" }] }
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockRoomodes
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
		...
		it("should return true when rules directory has content files", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.kitpilot/rules-test-mode" },
			])
		...
		it("should work with global custom modes when .kitpilotmodes doesn't exist", async () => {
			...
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath // .kitpilotmodes doesn't exist
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockSettingsPath) {
					return yaml.stringify(settingsContent)
	...
	describe("exportModeWithRules", () => {
		...
		it("should successfully export mode with rules for a custom mode in .kitpilotmodes", async () => {
			const kitpilotmodesContent = {
				customModes: [
					{
						slug: "test-mode",
		...
		it("should successfully export mode with rules for a built-in mode customized in .kitpilotmodes", async () => {
			const kitpilotmodesContent = {
				customModes: [
					{
						slug: "code",
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.kitpilot/rules-code" },
			])
		...
		it("should successfully export global mode with rules from global .kitpilot directory", async () => {
			// Mock a global mode
			const globalMode = {
