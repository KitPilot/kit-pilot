...
export async function parseMentions(
	...
	showKitPilotIgnoredFiles: boolean = false,
...
async function getFileOrFolderContentWithMetadata(
	...
	try {
		...
		if (stats.isFile()) {
			...
			if (kitpilotIgnoreController && !kitpilotIgnoreController.validateAccess(unescapedPath)) {
				return {
					type: "file",
					path: mentionPath,
					content: `[read_file for '${mentionPath}']\nNote: File is ignored by .kitpilotignore.`,
				}
			}
			try {
			...
			for (let index = 0; index < entries.length; index++) {
				...
				if (kitpilotIgnoreController) {
					isIgnored = !kitpilotIgnoreController.validateAccess(entryPath)
				}

				if (isIgnored && !showKitPilotIgnoredFiles) {
					continue
				}
