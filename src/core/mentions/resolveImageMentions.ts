...
export interface ResolveImageMentionsOptions {
	...
	images?: string[]
	cwd: string
	kitpilotIgnoreController?: { validateAccess: (filePath: string) => boolean }
 ...
 * - Respects `.kitpilotignore` via `kitpilotIgnoreController.validateAccess` when provided
 */
export async function resolveImageMentions({
	...
	for (const mention of imageMentions) {
		...
		if (kitpilotIgnoreController && !kitpilotIgnoreController.validateAccess(relPath)) {
			continue
		}
