...
describe("resolveImageMentions", () => {
	...
	it("should respect kitpilotIgnoreController", async () => {
		...
		const kitpilotIgnoreController = {
		...
		expect(kitpilotIgnoreController.validateAccess).toHaveBeenCalledWith("secret.jpg")
		expect(mockReadImageAsDataUrl).not.toHaveBeenCalled()
		expect(result.images).toEqual([])
	})
