...
import { isLanguage } from "@kit-pilot/types"
...
export async function loadRuleFiles(cwd: string, enableSubfolderRules: boolean = false): Promise<string> {
	...
	const kitpilotDirectories = enableSubfolderRules ? await getAllKitPilotDirectoriesForCwd(cwd) : getKitPilotDirectoriesForCwd(cwd)

	// Check for .kitpilot/rules/ directories in order (global, project-local, and optionally subfolders)
	for (const kitpilotDir of kitpilotDirectories) {
		const rulesDir = path.join(kitpilotDir, "rules")
	...
	// If we found rules in .kitpilot/rules/ directories, return them
	if (rules.length > 0) {
		return "\n# Rules from .kitpilot directories:\n\n" + rules.join("\n\n")
	}
...
/**
 * Load all AGENTS.md files from project root and optionally subdirectories with .kitpilot folders
...
async function loadAllAgentRulesFiles(cwd: string, enableSubfolderRules: boolean = false): Promise<string> {
	...
	// When enabled, load from root and all subdirectories with .kitpilot folders
...
export async function addCustomInstructions(
	...
	if (mode) {
		...
		// Use recursive discovery only if enableSubfolderRules is true
		const kitpilotDirectories = enableSubfolderRules
			? await getAllKitPilotDirectoriesForCwd(cwd)
			: getKitPilotDirectoriesForCwd(cwd)

		// Check for .kitpilot/rules-${mode}/ directories in order (global, project-local, and optionally subfolders)
		for (const kitpilotDir of kitpilotDirectories) {
			const modeRulesDir = path.join(kitpilotDir, `rules-${mode}`)
		...
		// If we found mode-specific rules in .kitpilot/rules-${mode}/ directories, use them
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		if (usedRuleFile.includes(path.join(".kitpilot", `rules-${mode}`))) {
	...
	if (options.kitpilotIgnoreInstructions) {
		rules.push(options.kitpilotIgnoreInstructions)
	}

	// Add AGENTS.md content if enabled (default: true)
	// Load from root and optionally subdirectories with .kitpilot folders based on enableSubfolderRules setting
