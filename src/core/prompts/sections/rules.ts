...
- For multi-step tasks (anything requiring more than one or two tool calls to complete), use the \`update_todo_list\` tool to outline your plan BEFORE making file modifications or running long-lived commands. This keeps your work focused and gives the user a checkpoint of what's coming. Skip this for trivial actions (a single read, a single edit, a quick answer) — process for its own sake is worse than no process.${settings?.isStealthModel ? getVendorConfidentialitySection() : ""}`
}
