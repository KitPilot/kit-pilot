// @ts-nocheck — vscode-lm-only build: this file is kept on disk per user
// choice ("keep but unregister"). The tool is not registered in build-tools
// and the OpenRouter handler it depended on has been removed. Re-enabling
// requires restoring an image-gen handler.
...
// vscode-lm-only build: image generation is unregistered (see build-tools).
// This file is kept on disk to make re-enabling easy if image-gen comes back.
