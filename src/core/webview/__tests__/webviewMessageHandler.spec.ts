...
describe("webviewMessageHandler - requestRouterModels", () => {
	...
	// vscode-lm-only build: router/dynamic providers were removed. The handler
	// is a stub that replies with an empty payload so the webview's loading
	// state resolves; it must never hit the model fetchers.
	it("replies with an empty routerModels payload without fetching", async () => {
		await webviewMessageHandler(mockClineProvider, {
	...
	it("echoes the requested provider back in values", async () => {
		await webviewMessageHandler(mockClineProvider, {
			type: "requestRouterModels",
...
describe("webviewMessageHandler - deleteCustomMode", () => {
	beforeEach(() => {
		...
		vi.mocked(ensureSettingsDirectoryExists).mockResolvedValue("/mock/global/storage/.kitpilot")
	})

	it("should delete a project mode and its rules folder", async () => {
		const slug = "test-project-mode"
		const rulesFolderPath = path.join("/mock/workspace", ".kitpilot", `rules-${slug}`)
	...
	it("should delete a global mode and its rules folder", async () => {
		...
		const rulesFolderPath = path.join(homeDir, ".kitpilot", `rules-${slug}`)
		...
		const getSkillsForMode = vi.fn().mockReturnValue([
			{
				...
				path: "/mock/.kitpilot/skills/skill-slug-entry/SKILL.md",
				source: "project",
				modeSlugs: ["code"],
			},
			{
				...
				path: "/mock/.kitpilot/skills/duplicate-skill/SKILL.md",
				source: "global",
				modeSlugs: ["code"],
			},
			{
				...
				path: "/mock/.kitpilot/skills/another-skill-slug/SKILL.md",
				source: "global",
				modeSlugs: ["code"],
			},
		])
			...
			expect.arrayContaining([
				{
					name: "skill-slug-entry",
					source: "project",
					filePath: "/mock/.kitpilot/skills/skill-slug-entry/SKILL.md",
					description: "Primary skill slug",
				},
				{
					name: "another-skill-slug",
					source: "global",
					filePath: "/mock/.kitpilot/skills/another-skill-slug/SKILL.md",
					description: "Another skill-generated command",
				},
			]),
		)
		...
		mockGetCommands.mockResolvedValue([
			{
				...
				source: "project",
				filePath: "/mock/workspace/.kitpilot/commands/deploy.md",
		...
		const getSkillsForMode = vi.fn().mockReturnValue([
			{
				...
				path: "/mock/.kitpilot/skills/deploy/SKILL.md",
				source: "global",
				modeSlugs: ["code"],
			},
			{
				...
				path: "/mock/.kitpilot/skills/skill-only/SKILL.md",
				source: "project",
				modeSlugs: ["code"],
			},
		])
		...
		expect(mockClineProvider.postMessageToWebview).toHaveBeenCalledWith({
			...
			commands: expect.arrayContaining([
				...
				},
				{
					name: "skill-only",
					source: "project",
					filePath: "/mock/.kitpilot/skills/skill-only/SKILL.md",
					description: "Skill-generated command",
				},
			]),
		})
