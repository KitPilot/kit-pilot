...
import { worktreeService, worktreeIncludeService, type CopyProgressCallback } from "@kit-pilot/core"
...
export async function handleSwitchWorktree(
	...
	try {
		...
		if (newWindow) {
			// Set the auto-open path so the new window opens KitPilot sidebar.
...
export async function handleGetWorktreeDefaults(provider: ClineProvider): Promise<WorktreeDefaultsResponse> {
	...
	const dotKitPilotPath = path.join(os.homedir(), ".kitpilot")
	const suggestedPath = path.join(dotKitPilotPath, "worktrees", `${projectName}-${suffix}`)

	return {
		suggestedBranch: `worktree/kitpilot-${suffix}`,
		suggestedPath,
	}
}
