...
// vscode-lm-only build: the OpenAI Compatible provider is removed. This stub
// preserves the message handler's shape so the webview can't crash if a stale
// requestOpenAiModels message arrives. It always returns an empty list.
...
export const webviewMessageHandler = async (provider: ClineProvider, message: WebviewMessage) => {
	...
	switch (message.type) {
		...
		case "requestRouterModels": {
			...
			// Reply with an empty payload so the webview's loading state
		...
		case "openKeyboardShortcuts": {
			// Open VSCode keyboard shortcuts settings and optionally filter to show the KitPilot commands
