...
export async function getEnvironmentDetails(cline: Task, includeFileDetails: boolean = false) {
	...
	const cache = cline.envDetailsCache
	const isFirstTurn = cache.lastTimeSentMs === 0
	...
	// Only include if changed since last turn (saves tokens on stable editors)
	if (allowedVisibleFiles && allowedVisibleFiles !== cache.visibleFiles) {
		details += "\n\n# VSCode Visible Files"
		details += `\n${allowedVisibleFiles}`
	}
	cache.visibleFiles = allowedVisibleFiles ?? ""
	...
	// Only include if changed since last turn
	if (allowedOpenTabs && allowedOpenTabs !== cache.openTabs) {
		details += "\n\n# VSCode Open Tabs"
		details += `\n${allowedOpenTabs}`
	}
	...
	// Only send on the first turn of the task, or every 5 minutes — the model
	// doesn't benefit from knowing the exact time on every single turn.
	const timeSinceLastSentMs = Date.now() - cache.lastTimeSentMs
	const shouldSendTime = includeCurrentTime && (isFirstTurn || timeSinceLastSentMs >= 5 * 60 * 1000)
	if (shouldSendTime) {
		const now = new Date()
	...
	// Only include mode section if it changed since last turn
	if (isFirstTurn || currentMode !== cache.modeSlug) {
		details += `\n\n# Current Mode\n`
		details += `<slug>${currentMode}</slug>\n`
		details += `<name>${modeDetails.name}</name>\n`
		details += `<model>${modelId}</model>\n`
	}
	cache.modeSlug = currentMode
	...
	// Update the timestamp so time-gated sections (e.g. current time) are
	// evaluated correctly on the next turn.
	cache.lastTimeSentMs = Date.now()
