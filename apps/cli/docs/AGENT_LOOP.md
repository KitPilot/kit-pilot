...
This document explains how the KitPilot CLI detects and tracks the agent loop state.
...
const client = new ExtensionClient({
	...
	debug: true, // Writes to ~/.kitpilot/cli-debug.log
})
...
The CLI uses type guards from `@kit-pilot/types` for categorization:

```typescript
import { isIdleAsk, isInteractiveAsk, isResumableAsk, isNonBlockingAsk } from "@kit-pilot/types"
...
Enable with `-d` flag. Logs go to `~/.kitpilot/cli-debug.log`:

```bash
kitpilot -d -P "Build something" --no-tui
```

View logs:

```bash
tail -f ~/.kitpilot/cli-debug.log
```
