#!/bin/bash
# KitPilot CLI Local Build Script
#
# Usage:
...
build() {
    ...
    pnpm --filter @kit-pilot/cli build
...
create_tarball() {
    ...
    RELEASE_DIR="$REPO_ROOT/kitpilot-cli-${PLATFORM}"
    ...
    cat > "$RELEASE_DIR/bin/kitpilot" << 'WRAPPER_EOF'
#!/usr/bin/env node
...
process.env.KITPILOT_CLI_ROOT = join(__dirname, '..');
process.env.KITPILOT_EXTENSION_PATH = join(__dirname, '..', 'extension');
const ripgrepPath = join(__dirname, 'rg');
if (existsSync(ripgrepPath)) {
  process.env.KITPILOT_RIPGREP_PATH = ripgrepPath;
}
...
WRAPPER_EOF
    ...
    chmod +x "$RELEASE_DIR/bin/kitpilot"
...
verify_local_install() {
    ...
    KITPILOT_LOCAL_TARBALL="$TARBALL_PATH" \
    KITPILOT_INSTALL_DIR="$VERIFY_INSTALL_DIR" \
    KITPILOT_BIN_DIR="$VERIFY_BIN_DIR" \
    KITPILOT_VERSION="$VERSION" \
    "$CLI_DIR/install.sh" || {
        rm -rf "$VERIFY_DIR"
    ...
    # Test --help
    if ! "$VERIFY_BIN_DIR/kitpilot" --help > /dev/null 2>&1; then
        rm -rf "$VERIFY_DIR"
    ...
    # Test --version
    if ! "$VERIFY_BIN_DIR/kitpilot" --version > /dev/null 2>&1; then
        rm -rf "$VERIFY_DIR"
...
print_summary() {
    ...
    if [ "$LOCAL_INSTALL" = true ]; then
        echo "  Installed to: ~/.kitpilot/cli"
        echo "  Binary: ~/.local/bin/kitpilot"
        echo ""
        echo "  Test it out:"
        echo "    kitpilot --version"
        echo "    kitpilot --help"
    else
        echo "  To install manually:"
        echo "    KITPILOT_LOCAL_TARBALL=$REPO_ROOT/$TARBALL ./apps/cli/install.sh"
        echo ""
