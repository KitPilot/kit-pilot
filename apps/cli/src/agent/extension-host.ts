/**
 * ExtensionHost - Loads and runs the KitPilot extension in CLI mode
 *
...
import { createVSCodeAPI, IExtensionHost, ExtensionHostEventMap, setRuntimeConfigValues } from "@kit-pilot/vscode-shim"
import { DebugLogger, setDebugLogEnabled } from "@kit-pilot/core/cli"
...
// When running from a release tarball, KITPILOT_CLI_ROOT is set by the wrapper script.
...
const CLI_PACKAGE_ROOT = process.env.KITPILOT_CLI_ROOT || findCliPackageRoot()
...
export interface ExtensionHostInterface extends IExtensionHost<ExtensionHostEventMap> {
	...
	runTask(prompt: string, taskId?: string, configuration?: KitPilotSettings, images?: string[]): Promise<void>
...
export class ExtensionHost extends EventEmitter implements ExtensionHostInterface {
	...
	private initialSettings: KitPilotSettings
	...
	constructor(options: ExtensionHostOptions) {
		...
		this.previousCliRuntimeEnv = process.env.KITPILOT_CLI_RUNTIME
		...
		const baseSettings: KitPilotSettings = {
	...
	public markWebviewReady(): void {
		...
		setRuntimeConfigValues("kit-pilot", this.initialSettings as Record<string, unknown>)
	...
	async dispose(): Promise<void> {
		...
		if (this.previousCliRuntimeEnv === undefined) {
			...
			process.env.KITPILOT_CLI_RUNTIME = this.previousCliRuntimeEnv
		}
	}
}
