...
import { getGitSha, copyPaths, copyLocales, copyWasms, generatePackageJson } from "@kit-pilot/build"
...
async function main() {
	...
	const buildOptions = {
		...
		platform: "node",
		define: {
			"process.env.PKG_NAME": '"kit-pilot-nightly"',
			"process.env.PKG_VERSION": `"${overrideJson.version}"`,
			"process.env.PKG_OUTPUT_CHANNEL": '"KitPilot-Nightly"',
