...
import { KitPilotEventName, type ClineMessage } from "@kit-pilot/types"

import { waitFor, sleep } from "../utils"
import { setDefaultSuiteTimeout } from "../test-utils"

suite.skip("KitPilot use_mcp_tool Tool", function () {
	...
	suiteSetup(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kitpilot-test-mcp-"))
		...
		testFiles = {
			...
			mcpConfig: path.join(workspaceDir, ".kitpilot", "mcp.json"),
		}
		...
		// Create .kitpilot directory and MCP configuration file
		const kitpilotDir = path.join(workspaceDir, ".kitpilot")
	...
	suiteTeardown(async () => {
		...
		// Clean up .kitpilot directory
		const workspaceDir = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath || tempDir
		const kitpilotDir = path.join(workspaceDir, ".kitpilot")
		try {
			await fs.rm(kitpilotDir, { recursive: true, force: true })
		} catch {
	...
	test("Should request MCP filesystem read_file tool and complete successfully", async function () {
		...
		api.on(KitPilotEventName.Message, messageHandler)
		...
		api.on(KitPilotEventName.TaskStarted, taskStartedHandler)
		...
		api.on(KitPilotEventName.TaskCompleted, taskCompletedHandler)
		await sleep(2000) // Wait for KitPilot to fully initialize
		...
		try {
			...
			// Make a small modification to trigger the save event, without this KitPilot won't load the MCP server
		...
		try {
			...
			// Clean up
			api.off(KitPilotEventName.Message, messageHandler)
			api.off(KitPilotEventName.TaskStarted, taskStartedHandler)
			api.off(KitPilotEventName.TaskCompleted, taskCompletedHandler)
		}
	})
	...
	test("Should request MCP filesystem list_directory tool and complete successfully", async function () {
		...
		try {
			...
			const hasKitPilotDir = responseText.includes(".kitpilot")

			// At least one of our test files or the .kitpilot directory should be present
			assert.ok(
				hasTestFile || hasDataFile || hasKitPilotDir,
				`MCP server response should contain our test files or .kitpilot directory. Expected to find: '${path.basename(testFiles.simple)}', '${path.basename(testFiles.testData)}', or '.kitpilot'. Got: ${responseText.substring(0, 200)}...`,
			)
	...
	test.skip("Should request MCP filesystem directory_tree tool and complete successfully", async function () {
		...
		try {
			...
			const hasTestFiles =
				...
				responseText.includes(".kitpilot") ||
				responseText.includes(".txt") ||
				responseText.includes(".json") ||
