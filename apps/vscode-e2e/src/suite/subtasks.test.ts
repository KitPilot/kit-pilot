...
suite.skip("KitPilot Subtasks", () => {
	test("Should handle subtask cancellation and resumption correctly", async () => {
		const api = globalThis.api

		const messages: Record<string, ClineMessage[]> = {}

		api.on(KitPilotEventName.Message, ({ taskId, message }) => {
		...
		api.on(KitPilotEventName.TaskSpawned, (_, childTaskId) => (spawnedTaskId = childTaskId))
