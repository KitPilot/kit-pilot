...
export default defineConfig(({ mode }) => {
	...
	const define: Record<string, any> = {
		...
		"process.env.PKG_OUTPUT_CHANNEL": JSON.stringify("KitPilot"),
		...(gitSha ? { "process.env.PKG_SHA": JSON.stringify(gitSha) } : {}),
	}

	// TODO: We can use `@kit-pilot/build` to generate `define` once the
	...
	if (mode === "nightly") {
		...
		define["process.env.PKG_OUTPUT_CHANNEL"] = JSON.stringify("KitPilot-Nightly")
	}
	...
	return {
		...
		resolve: {
			alias: {
				...
				"@kitpilot": resolve(__dirname, "../src/shared"),
			},
		},
		build: {
			outDir,
			...
			rollupOptions: {
				...
				// dynamically imported by kitpilot-config/index.ts, but should never be bundled
